...
var rootURL = "http://localhost:8080/bookStore_Project3/rest/bookService";
...
$('#btnSearch').click(function() {
	search($('#author_FirstName').val(),$('#author_LastName').val());
	return false;
});
...
function search(author_FirstName,author_LastName) {
	$.ajax({
		type: 'GET',
		url: rootURL + '/searchBook/' + author_FirstName+'/'+author_LastName,
		dataType: "json",
		success: renderList 
	});
}
...
function renderList(data) {
	...
	$.each(list, function(index, book) {
		$('#search').append('<td>'+book.book_name+'</td>');
	});
}
